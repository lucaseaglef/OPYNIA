...
import { Calendar, Star, MessageSquare, Download, Search } from "lucide-react"
...
export function ResponsesTable({ survey, responses }: ResponsesTableProps) {
  ...
  const exportTableCSV = () => {
    const headers = ["Data", ...survey.fields.filter((f) => f.type !== "divider").map((f) => f.label)]
    ...
    link.download = `FEIND_Tabela_${new Date().toISOString().split("T")[0]}.csv`
    link.click()
  }
      ...
      <Card className="modern-card">
        ...
        <CardContent>
          <div className="flex flex-col sm:flex-row gap-4">
            ...
            <Button variant="outline" onClick={exportTableCSV}>
                    ...
                    <tr key={response.id} className="hover:bg-gray-50">
                      ...
                      {mainFields.map((field) => (
                        <td key={field.id} className="px-4 py-3">
                          {renderAnswer(field, response.answers[field.id])}
                        </td>
                      ))}
                    </tr>
                  ))}
