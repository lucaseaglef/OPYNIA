...
import { Plus, Trash2, GripVertical, Save, ArrowLeft, Settings, Eye, Edit3, LinkIcon, Copy } from "lucide-react"
...
export default function EditSurvey() {
  ...
  const saveSurvey = async () => {
    ...
    if (!customSlug) {
      alert("Por favor, defina uma URL válida para a pesquisa.")
      return
    }
    ...
    try {
      ...
      if (customSlug !== surveyId) {
        const saveSuccess = await SurveyStorage.saveSurvey(updatedSurvey)
        if (saveSuccess) {
        ...
        const success = await SurveyStorage.updateSurvey(surveyId, updatedSurvey)
        if (success) {
          alert("Pesquisa salva com sucesso!")
        } else {
          alert("Erro ao salvar pesquisa. Tente novamente.")
    ...
    return (
      <div className="min-h-screen bg-[#121826] flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-xl text-gray-300 font-medium">Carregando editor...</p>
        </div>
      </div>
    )
  }
  ...
  if (!survey) {
    return (
      <div className="min-h-screen bg-[#121826] flex items-center justify-center">
        <Card className="w-full max-w-md bg-[#1e293b] border-orange-500/30">
          <CardContent className="text-center py-12">
            <h2 className="text-xl font-semibold text-white mb-2">Pesquisa não encontrada</h2>
            <p className="text-gray-400">A pesquisa que você está procurando não existe.</p>
            <Link href="/" className="mt-4 inline-block">
              <Button className="bg-orange-500 hover:bg-orange-600">Voltar ao Dashboard</Button>
            </Link>
          </CardContent>
        </Card>
      </div>
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-4">
              ...
              <div>
                <h1 className="text-2xl font-bold text-white flex items-center">
                  ...
                  {survey.title || "Editor de Pesquisa"}
                </h1>
            ...
            <div className="flex items-center space-x-3">
              <Button
                ...
                variant="outline"
                size="sm"
                className="border-orange-300 text-orange-400 hover:bg-orange-50 border-orange-500/30"
              >
        ...
        <div className="flex-1 flex flex-col">
          ...
          <div className="bg-[#1e293b] border-b border-gray-700 p-4">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              <div>
                <Label className="text-xs text-gray-400">URL da Pesquisa</Label>
                <div className="flex mt-1">
                  <span className="bg-[#2a3548] border border-r-0 border-gray-600 px-2 py-2 text-xs text-gray-400 rounded-l">
                    /survey/
                  </span>
                  <Input
                    value={customSlug}
                    onChange={(e) => handleSlugChange(e.target.value)}
                    placeholder="url-da-pesquisa"
                    className="rounded-l-none bg-[#2a3548] border-gray-600 text-white"
                  />
                </div>
              </div>
